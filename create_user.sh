...
# Define External Access Integration name
...
if [ "$SHOW_CONFIG" = true ]; then
    ...
    echo "  Ext. Access:    $EXTERNAL_ACCESS_NAME"
    if [ -n "$ENV_PREFIX" ]; then
...
${COMPUTE_POOL_GRANTS}
-- ============================================================
-- 9. GRANT CORTEX LLM ACCESS
-- ============================================================

GRANT DATABASE ROLE SNOWFLAKE.CORTEX_USER TO ROLE ${SNOWFLAKE_ROLE};

-- Grant access to Cortex Search Services (if they exist)
GRANT USAGE ON ALL CORTEX SEARCH SERVICES IN SCHEMA ${SNOWFLAKE_DATABASE}.${SNOWFLAKE_SCHEMA} TO ROLE ${SNOWFLAKE_ROLE};
GRANT USAGE ON FUTURE CORTEX SEARCH SERVICES IN SCHEMA ${SNOWFLAKE_DATABASE}.${SNOWFLAKE_SCHEMA} TO ROLE ${SNOWFLAKE_ROLE};

-- ============================================================
-- 10. GRANT INTEGRATION ACCESS
-- ============================================================

GRANT USAGE ON INTEGRATION ${EXTERNAL_ACCESS_NAME} TO ROLE ${SNOWFLAKE_ROLE};
